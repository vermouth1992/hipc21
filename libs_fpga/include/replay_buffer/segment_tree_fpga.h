...
//            throw NotImplemented();
            for (int i = 0; i < insert_batch; i++) {
                // printf("\njj,i:%d,%d\n",jj,i);
                rlu::fpga::insert_ind[i] = idx.index({i}).item<int>(); 
                rlu::fpga::init_priority[i] = value.index({i}).item<float>();
                // rlu::fpga::q.finish();

            }
            cl::Kernel krnl_tree6(rlu::fpga::program, "Top_tree");
            ...
            krnl_tree6.setArg(0, rlu::fpga::insert_signal_in);
            krnl_tree6.setArg(1, rlu::fpga::insind_buf);
            krnl_tree6.setArg(2, rlu::fpga::inpn_buf);
            krnl_tree6.setArg(3, rlu::fpga::update_signal);
        ...
        [[nodiscard]] torch::Tensor sample_idx(int64_t batch_size) const override { //Int32???????
            ...
            // spdlog::info("Replay sample: setArg done");
            // rlu::fpga::q.enqueueMigrateMemObjects({rlu::fpga::insind_buf,rlu::fpga::inpn_buf}, 0);
            // rlu::fpga::q.finish();
            // spdlog::info("Replay sample: data transfer to device");
            // rlu::fpga::q.enqueueTask(krnl_tree2);
            // rlu::fpga::q.finish();
            // rlu::fpga::q.enqueueMigrateMemObjects({rlu::fpga::out_buf}, CL_MIGRATE_MEM_OBJECT_HOST);
            // spdlog::info("Replay sample: data transfer to host done");
            // rlu::fpga::q.finish();
